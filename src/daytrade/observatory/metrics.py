...
_CALIBRATION_BUCKETS = [(0.50, 0.60), (0.60, 0.70), (0.70, 0.80), (0.80, 0.90), (0.90, 1.01)]
...
def regime_metrics(
    outcomes: List[Dict[str, Any]], regime_rows: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """Per-regime performance: accuracy, fake PnL, sample counts."""
    per: Dict[str, Dict[str, float]] = {}
    for o in outcomes:
        cond = o.get("market_condition") or "UNKNOWN"
        agg = per.setdefault(cond, {"samples": 0.0, "correct": 0.0, "fake_pnl": 0.0})
        if o.get("directionally_correct") is not None:
            agg["samples"] += 1
    ...
    by_regime = {
        cond: {
            ...
            "accuracy": round(a["correct"] / a["samples"] * 100, 1) if a["samples"] else None,
...
def roll_up_day(db, day_date: str, day_number: int, expected_cycles: int) -> Dict[str, Any]:
    ...
    errors = [
        e
        for e in _on(db.recent_errors(limit=5000))
    ...
    accuracy = (
        (sum(int(o["directionally_correct"]) for o in scored) / len(scored)) if scored else 0.0
    )
    ...
    return {
        ...
        "dominant_condition": (conditions.most_common(1)[0][0] if conditions else None),
        "errors": len(errors),
        "status": status,
    }
...
def learning_metrics(db) -> Dict[str, Any]:
    ...
    fp = sum(1 for o in scored if o.get("direction") == "buy" and not o["directionally_correct"])
    fn = sum(1 for o in scored if o.get("direction") == "sell" and not o["directionally_correct"])
    ...
    return {
        "prediction": {
            ...
            "target_hit_rate": round(target_hits / len(outcomes) * 100, 1) if outcomes else 0.0,
            "stop_hit_rate": round(stop_hits / len(outcomes) * 100, 1) if outcomes else 0.0,
        ...
        "trading_simulation": {
            ...
            "profit_factor": round(gross_win / gross_loss, 2) if gross_loss > 1e-9 else 0.0,
        ...
        "market_understanding": {
            ...
            "panic_symbols": sum(1 for h in health if h.get("status") == "PANIC"),
            "liquidity_rejections": sum(1 for h in health if h.get("status") == "TOO ILLIQUID"),
            "choppy_rejections": sum(1 for h in health if h.get("status") == "TOO CHOPPY"),
        },
        "reliability": {

...
class DaytradeStrategy(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame,
        ...
        """
        df = dataframe
        # Freqtrade 2026+ uses pyarrow-backed dtypes that sometimes load
        # numeric columns as strings. Cast OHLCV to float defensively so
        # downstream numpy ufuncs (np.sign, comparisons) don't trip on
        # 'str < int' TypeErrors.
        ...
        # talib's MACD wrapper returns a DataFrame with columns
        # ['macd', 'macdsignal', 'macdhist'] — unpacking it would give
        # the column NAMES, not values. Index explicitly.
        macd_df = ta.MACD(df, fastperiod=12, slowperiod=26, signalperiod=9)
        df["macd"] = macd_df["macd"].astype("float64")
        df["macd_signal"] = macd_df["macdsignal"].astype("float64")
        df["macd_hist"] = macd_df["macdhist"].astype("float64")

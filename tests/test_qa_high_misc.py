...
def test_total_realised_pnl_sums_all_closed_trades(tmp_path):
    ...
    try:
        for i, pnl in enumerate([1.0, -0.5, 2.5, -1.0, 0.25]):
            ...
            db.close_paper_trade(row[0], exit_price=101.0, pnl=pnl, fees=0.1, slippage=0.05)
...
def test_upsert_outcome_and_mark_evaluated_atomic(tmp_path):
    ...
    try:
        ...
        db.upsert_outcome_and_mark_evaluated(pid, target_hit=1, stop_hit=0, realized_pnl=1.0)
        pred = db._one("SELECT evaluated FROM predictions WHERE id=?", (pid,))
        assert pred["evaluated"] == 1
        outcome = db._one(
            "SELECT prediction_id FROM prediction_outcomes WHERE prediction_id=?", (pid,)
        )
...
def test_prune_old_drops_aged_activity_rows(tmp_path):
    ...
    try:
        ...
        db._insert("activity_events", {"ts": old_ts, "event": "old", "detail": "", "level": "info"})
        # Recent row
        new_ts = (datetime.now(timezone.utc) - timedelta(hours=1)).isoformat()
        db._insert("activity_events", {"ts": new_ts, "event": "new", "detail": "", "level": "info"})

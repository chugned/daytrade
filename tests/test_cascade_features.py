...
from daytrade.config.schema import FeatureConfig, IndicatorConfig
from daytrade.features.pipeline import (
    FeaturePipeline, compute_features, feature_columns,
)
...
def _calm(n: int = 40, *, price: float = 100.0,
...
def test_cascade_columns_present_in_feature_columns():
    ...
    for name in ("cascade_body_atr", "cascade_vol_spike",
                 "cascade_lower_wick", "cascade_active",
                 "cascade_exhaustion"):
...
def test_active_flag_off_when_only_volume_spikes():
    ...
    bar = _candle(  # big volume, tiny body
        len(base), o=last, c=last * 0.9998, vol=20000.0,
    )
    pipe = FeaturePipeline()
...
def test_active_flag_off_when_only_body_drops():
    ...
    bar = _candle(  # big body, baseline volume
        len(base), o=last, c=last * 0.97, vol=1100.0,
        h=last * 1.0005, l=last * 0.969,
    )
    pipe = FeaturePipeline()
    feats = pipe.transform(base + [bar])
...
def test_exhaustion_fires_when_long_wick_follows_cascade():
    ...
    exhaustion_bar = _candle(
        len(base) + 1, o=px * 1.000, c=px * 1.002, vol=6000.0,
        h=px * 1.003, l=px * 0.980,
    )
    pipe = FeaturePipeline()
...
def test_exhaustion_does_not_fire_without_prior_cascade():
    ...
    bar = _candle(
        len(base), o=last, c=last * 1.001, vol=6000.0,
        h=last * 1.002, l=last * 0.985,
    )
    pipe = FeaturePipeline()
    feats = pipe.transform(base + [bar])
...
def _continuing_calm(start_i: int, n: int, *, price: float,
                     volume: float = 1000.0) -> List[OHLCV]:
...
def test_cascade_columns_are_causal():
    ...
    cascade_bar = _candle(
        len(base), o=last_px, c=last_px * 0.97, vol=8000.0,
        h=last_px * 1.0005, l=last_px * 0.969,
    )
    after = _continuing_calm(len(base) + 1, n=20,
                             price=cascade_bar.close, volume=1000.0)
    ...
    cols = ["cascade_body_atr", "cascade_vol_spike", "cascade_lower_wick",
    ...
    assert np.array_equal(common_full.values, common_partial.values,
                          equal_nan=False)

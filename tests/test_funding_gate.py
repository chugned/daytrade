...
def test_thresholds_are_configurable():
    block, _ = extreme_funding_blocks_buy(0.00015, extreme_positive=0.0001, extreme_negative=-0.001)
    assert block is True
...
def test_fetch_uses_cache_on_repeated_calls():
    fake = MagicMock()
    fake.status_code = 200
    fake.json.return_value = {"lastFundingRate": "0.00012", "markPrice": "100", "time": 1}
    client = MagicMock()
    client.get.return_value = fake
